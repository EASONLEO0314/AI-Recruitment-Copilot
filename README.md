...
- 导入质量报告：<http://127.0.0.1:8765/v1/knowledge/quality>
- 岗位详情与画像：<http://127.0.0.1:8765/v1/knowledge/jobs/job-001>

当前实现是确定性的“小型检索库”：它把岗位 JD、岗位关键词、核心产出、部门和项目等字段整理进本地 SQLite，并抽取标准化技能/业务概念、岗位画像和导入质量报告，便于后续评分、后台管理和候选人技能归一化。它还不是向量库；后续如需升级为 RAG，可以直接使用数据库中的 `documents` 表作为 embedding 输入。
...
生成结果写入 `backend/app/data/job_knowledge_base.sqlite3`。该文件保留在本机，不提交到 Git；生成文件不包含直属负责人、招聘负责人和招聘成本记录等内部字段。
...
- [ ] 停止后端，再点击在线状态下的“刷新连接”，悬浮窗清除旧结果并显示评分服务未连接及启动命令。
