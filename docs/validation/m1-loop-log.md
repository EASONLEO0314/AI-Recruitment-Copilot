...
## 9. 2026-07-29 Message tab click-target follow-up

人工试用报告“电话沟通提纲”标签点击范围不稳定。当前源码中的标签按钮高度为 33px，且没有整列宽度、独立层级和显式命中规则。本轮按新问题执行定向修复：

- 新增样式契约测试，要求每个标签覆盖完整 grid 列、最小高度 44px，并显式设置定位层级和 `pointer-events: auto`。修复前测试稳定失败，修复后通过。
- 三个标签改为宽度 100%、最小高度 44px，并使用 flex 居中内容。
- 组件测试增加对“电话沟通提纲”的直接点击，验证对应内容出现且不会自动复制。
- 主机环境执行一次 `npm.cmd run verify`：后端 12 passed；扩展 6 个测试文件、24 passed；TypeScript 类型检查和双入口构建通过。

新构建已经生成。首次只刷新网页时，Chrome 仍报告旧样式（33px、`position: static`），说明加载中的 unpacked 扩展尚未刷新。人工在扩展管理页刷新扩展并刷新验收页后，真实 Chrome 复验结果如下：

- 三个话术标签的计算样式均为最小高度 44px、`position: relative`、`z-index: 1`、`pointer-events: auto`。
- “电话沟通提纲”标签中间位置点击成功，标签进入选中状态并显示 4 条电话沟通步骤。
- 先切回“打招呼话术”，再分别点击“电话沟通提纲”标签的左侧和右侧区域；两次均成功切换到电话提纲，选中下划线和 4 条步骤均可见。

因此，本轮报告的“标签中间点不到”场景已在刷新后的真实 Chrome 中通过；此结论只覆盖该点击范围问题，不替代 M1 其余人工验收项。
