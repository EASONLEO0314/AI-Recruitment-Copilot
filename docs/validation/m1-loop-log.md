...
新构建已经生成。当前 Chrome 在只刷新网页后仍报告旧样式（33px、`position: static`），说明加载中的 unpacked 扩展尚未刷新。由于 Chrome 内部扩展管理页不能由自动化控制，修复后的真实鼠标验收必须在人工点击扩展刷新并刷新验收页后执行；在此之前不能把该人工场景标记为通过。
